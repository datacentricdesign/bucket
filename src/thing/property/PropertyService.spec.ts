...
describe("PropertyService", () => {
  ...
  it("Create property", async function () {
    ...
    const propertyWithRetrievedValues =
      await propertyService.getOnePropertyById(
        createdThing.id,
        createdProperty.id,
        valueOptions
      );
    expect(propertyWithRetrievedValues.values.length).to.equal(
      4,
      "Number of values should be 4."
    );

    // test delete two data points

    await propertyService.deleteDataPoints(
      createdThing.id,
      createdProperty.id,
      [2, 4]
    );

    const propertyWithValuesAfterDelete =
      await propertyService.getOnePropertyById(
        createdThing.id,
        createdProperty.id,
        valueOptions
      );
    expect(propertyWithValuesAfterDelete.values.length).to.equal(
      2,
      "Number of values should be 2."
    );

    // test delete property

    await propertyService.deleteOneProperty(
      createdThing.id,
      createdProperty.id
    );

    try {
      const propertyAferDeletion = await propertyService.getOnePropertyById(
        createdThing.id,
        createdProperty.id
      );
      expect(propertyAferDeletion).to.be(
        undefined,
        "Property after deletion should be undefined."
      );
    } catch (error) {
    ...
    const values = await influxDBService.readValuesFromInfluxDB(
      createdProperty,
      valueOptions
    );
    expect(values.length).to.equal(
      0,
      "There should be no values after deletion."
    );
