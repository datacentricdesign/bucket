      ...
      if (!streams[0] || (streams[0] && streams[0].size !== size)) {
        ...
        stream.proc = ffmpeg()
          ...
          .on('end', () => {
            ...
            this.currentPropertyValue.push(Date.now() - parseInt(this.currentPropertyValue[0].toString()))
          })
        ...
        streams.forEach(stream => {
          if (stream.recordEnd) {
            ...
            if (totalEnd === streams.length) {
              ...
              const outputName = connection.property.thing.id + '-' +
                    connection.property.id + '-' +
                    connection.currentPropertyValue[0] + '#video-mp4.mp4';
              ...
              const mergeProc = ffmpeg()
                ...
                .on('end', () => {
                  ...
                  Log.debug('Merge end. You can play ' + connection.property.id);
                  // TODO push update property
                  connection.currentPropertyValue.push(outputName)
                  connection.property.values = [connection.currentPropertyValue]
                  PropertyController.propertyService.updatePropertyValues(connection.property)
                });
