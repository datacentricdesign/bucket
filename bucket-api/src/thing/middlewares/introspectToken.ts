...
export const introspectTokenWs = (requiredScope: string[]): WebsocketRequestHandler => {
    return async (ws: ws, req: Request, next: NextFunction) => {
        return _introspectToken(requiredScope, req, next)
    }
};

async function _introspectToken(requiredScope: string[], req: Request, next: NextFunction) {
